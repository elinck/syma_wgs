...
system("bbmask.sh in=B10K-DU-024-03.genomic.fa out=
B10K-DU-024-03.genomic.masked.fa entropy=0.7")

#run bbduk to trim adapters
R1 <- list.files("/media/burke/bigMac/ethan/syma_raw",full.names=T) %>% grep("R1_001",.,value=T)
R2 <- list.files("/media/burke/bigMac/ethan/syma_raw",full.names=T) %>% grep("R2_001",.,value=T)
commands <- c()
for(i in 1:20){
  commands[i] <- paste0("bbduk.sh in1=", R1[i],
                        " in2=",R2[i],
                        " out=", R1[i] %>% basename() %>% strsplit("_L006_R._001") %>% unlist() %>% .[1],"trimmed.fq",
